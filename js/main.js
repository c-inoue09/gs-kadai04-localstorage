...
$(document).on('click','td', function() {
   ...
   $('.modal-date').html(`<h2>20${yymmdd.slice(7,9)}年${yymmdd.slice(9,11)}月${yymmdd.slice(-2)}日のコーディネート</h2>`);
    ...
    $(document).on('click','.save-button', function() { //セーブボタンがおされたら以下実行
        console.log("セーブされました");

        const outfitForTheDay = {
            // occasionForTheDay: occasion
            topsForTheDay: topPic,
            bottomsForTheDay: bottomPic
        }

        console.log(outfitForTheDay);

        const jsonOutfitForTheDay = JSON.stringify(outfitForTheDay);
        localStorage.setItem(`outfitFor${yymmdd}`,jsonOutfitForTheDay);
 
        const savedOutfit = localStorage.getItem(`outfitFor${yymmdd}`);
        const parsedOutfitForTheDay = JSON.parse(savedOutfit);
        const topPicForTheDay = parsedOutfitForTheDay.topsForTheDay;
        const bottomPicForTheDay = parsedOutfitForTheDay.bottomsForTheDay;

        console.log(topPicForTheDay);
        console.log(yymmdd);

        const yymmddMin = yymmdd.replace('outfit-','' );
        
        $(`#item-${yymmddMin}-top`).html(topPicForTheDay); // 画像をカレンダー内のdiv(id: item-YYMMDD-top)に格納
        $(`#item-${yymmddMin}-bottom`).html(bottomPicForTheDay); // // 画像をカレンダー内の#item-YYMMDD-bottomに格納
        $('.modal-container').removeClass('active'); //モーダル閉じる
       });
